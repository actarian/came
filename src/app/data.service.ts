...
export default class MtmDataService {
	...
	static partsKeys: string[] = ['electronicsModule1', 'electronicsModule2', 'electronicsModule3', 'electronicsModule4',
		'frontPiece1', 'frontPiece2', 'frontPiece3', 'frontPiece4',
		'frame', 'mounting', 'flushRainshield'];
	static partsPool: { [key: number]: MtmPart } = {};
	...
	static fetchJson(callback?: Function, error?: Function) {
		...
		return Promise.all(
			...
			.then(all => {
				const parts = all[1];
				const localizations = all[2];
				const partsPool = this.partsPool;
